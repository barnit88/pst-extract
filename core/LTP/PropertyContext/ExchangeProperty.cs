        ...
        private void GetData(BTH heap, bool isTable = false)
        {
            ...
            if (!this.MultiValue && !this.Variable)
            {
                if (this.ByteCount <= 4 || (isTable && this.ByteCount <= 8))
                    ...
                    this.Data = HeapOnNode.GetHNHIDBytes(heap.HeapOnNode, new HID(this.Key));
            }
            else
            {
                ...
                else
                {
                    ...
                    var subNode = heap.HeapOnNode.Node.SubNodeData.FirstOrDefault(n => n.NodeID == curID);
                    if (subNode != null)
                        dataBlocks = subNode.NodeData;
                    else
                    {
                        var temp = heap.HeapOnNode.Node.SubNodeData.FirstOrDefault(n => (n.NodeID & 0xFFFFFFFF) == curID);
                        if (temp != null)
                            dataBlocks = temp.NodeData;
                    }
